...
// ==========================================================================================
// SECTION 1: CORE UTILITIES & HASH TABLE (Unchanged and Correct)
// ==========================================================================================
...
// ==========================================================================================
// SECTION 2: RADIX PARTITIONING (FINAL FIX - USING SAFE SCALAR WRITES)
// ==========================================================================================
inline size_t get_partition_idx(uint64_t hash) { return hash >> (64 - RADIX_BITS); }

// The buffer size can be a small number, its main goal is to batch atomic operations.
constexpr size_t PARTITION_BUFFER_SIZE = 8; 

template<typename T>
struct PartitionBuffer {
    std::array<T, PARTITION_BUFFER_SIZE> data;
    uint8_t count = 0;
};
...
parallel_radix_partition_kv(const uint64_t* keys, const uint64_t* values, size_t size, size_t num_threads) {
    ...
    for (auto& t : threads) { t.join(); }
    for (size_t i = 0; i < num_threads; ++i) { for (size_t j = 0; j < NUM_PARTITIONS; ++j) { partition_offsets[j + 1] += histograms[i][j]; } }
    ...
    for (size_t i = 0; i < num_threads; ++i) {
        ...
        threads.emplace_back([&, start, end]() {
            auto key_buffers = std::make_unique<std::vector<PartitionBuffer<uint64_t>>>(NUM_PARTITIONS);
            auto val_buffers = std::make_unique<std::vector<PartitionBuffer<uint64_t>>>(NUM_PARTITIONS);
            for (size_t j = start; j < end; ++j) {
                const uint64_t key = keys[j]; const uint64_t val = values[j];
                const size_t p_idx = get_partition_idx(hasher(key));
                auto& kbuf = (*key_buffers)[p_idx]; auto& vbuf = (*val_buffers)[p_idx];
                kbuf.data[kbuf.count] = key; vbuf.data[vbuf.count] = val;
                kbuf.count++; vbuf.count++;
                if (kbuf.count == PARTITION_BUFFER_SIZE) {
                    size_t write_pos = atomic_write_offsets[p_idx].fetch_add(PARTITION_BUFFER_SIZE, std::memory_order_relaxed);
                    // BUG FIX: Removed all streaming store intrinsics.
                    // Use a simple, safe scalar loop. The compiler will optimize this effectively.
                    for (size_t k = 0; k < PARTITION_BUFFER_SIZE; ++k) {
                        out_keys[write_pos + k] = kbuf.data[k];
                        out_values[write_pos + k] = vbuf.data[k];
                    }
                    kbuf.count = 0; vbuf.count = 0;
                }
            }
            // Flush remaining items
            for (size_t p_idx = 0; p_idx < NUM_PARTITIONS; ++p_idx) {
                auto& kbuf = (*key_buffers)[p_idx];
                if (kbuf.count > 0) {
                    auto& vbuf = (*val_buffers)[p_idx];
                    size_t write_pos = atomic_write_offsets[p_idx].fetch_add(kbuf.count, std::memory_order_relaxed);
                    for (size_t k = 0; k < kbuf.count; ++k) {
                        out_keys[write_pos + k] = kbuf.data[k]; out_values[write_pos + k] = vbuf.data[k];
                    }
                }
            }
        });
    ...
    for (size_t i = 0; i < num_threads; ++i) {
        ...
        threads.emplace_back([&, start, end]() {
            ...
            for (size_t j = start; j < end; ++j) {
                ...
                const size_t p_idx = get_partition_idx(hasher(key));
                auto& kbuf = (*key_buffers)[p_idx];
                kbuf.data[kbuf.count] = key;
                kbuf.count++;
                if (kbuf.count == PARTITION_BUFFER_SIZE) {
                    size_t write_pos = atomic_write_offsets[p_idx].fetch_add(PARTITION_BUFFER_SIZE, std::memory_order_relaxed);
                    // BUG FIX: Use a simple, safe scalar loop.
                    for (size_t k = 0; k < PARTITION_BUFFER_SIZE; ++k) {
                        out_keys[write_pos + k] = kbuf.data[k];
                    }
                    kbuf.count = 0;
            ...
            for (size_t p_idx = 0; p_idx < NUM_PARTITIONS; ++p_idx) {
                ...
                if (kbuf.count > 0) {
                    size_t write_pos = atomic_write_offsets[p_idx].fetch_add(kbuf.count, std::memory_order_relaxed);
                    for (size_t k = 0; k < kbuf.count; ++k) { out_keys[write_pos + k] = kbuf.data[k]; }
                }
            }
        });
    }
...
// ==========================================================================================
// SECTION 3 & 4: JOIN FUNCTIONS & PYBIND MODULE (Unchanged and Correct)
// ==========================================================================================
